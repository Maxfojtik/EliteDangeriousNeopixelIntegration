const int numSections = NUMPIXELS/4;
...
int minSize = NUMPIXELS/numSections/2;
int goodTime = -1;
...
void loadStation()
{
  if(notificationVar==0)
  {
    ...
    {
      int ran = -1;
      while(abs(closestSplit(ran)-ran)<minSize || ran<minSize || ran>(strip.numPixels()-minSize) || ran==-1)
      {
        ran = random(0,strip.numPixels());
      }
      splits[i] = ran;
    }
...
//    Serial.println(brights[splitNum]);
    strip.setPixelColor(i, cRed*brights[splitNum]/255.0, cGreen*brights[splitNum]/255.0, cBlue*brights[splitNum]/255.0);
  }
  notificationVar++;
